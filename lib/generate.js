...
function standardtable() {
  ...
  inquirer.prompt(questions).then(function (answers) {
    ...
    setTargetPath(function (target) {
      try {
        fs.writeFileSync(`${target}/${name}/Index.js`, tableTemplate.routerPage({ name: name }), 'utf8');
    ...
    setTargetPath(function (target) {
      try {
        fs.writeFileSync(`${target}/${name}/Index.less`, tableTemplate.routerPageLess({ name: name }), 'utf8');
    ...
    setTargetPath(function (target) {
      try {
        fs.writeFileSync(`${target}/${name}/ModalDetailForm.js`, tableTemplate.modalDetailFormPage({ name: name }), 'utf8');
    ...
    setTargetPath(function (target) {
      try {
        fs.writeFileSync(`${target}/${name}/pageConfig.js`, tableTemplate.pageConfig({ name: name }), 'utf8');
    ...
    setTargetPath(function (target) {
      try {
        fs.writeFileSync(`${target}/${name}.js`, tableTemplate.model({ name: name }), 'utf8');
        console.log(chalk.green(`generated model ${name} success：${path.join(target, name + '.js')}`));
      } catch (e) {
        console.log(chalk.red(`generated model ${name} fail`, e));
      }
    ...
    setTargetPath(function (target) {
      try {
        fs.writeFileSync(`${target}/${name}.js`, tableTemplate.mockData({ name: name }), 'utf8');
    ...
    '/${name}/${name}': {
      name: '标准列表页',
      component: dynamicWrapper(app, ['${name}', 'dictionary'], () => import('../routes/${name}/Index')),
    },
    `));
    console.log('-------------------------');
